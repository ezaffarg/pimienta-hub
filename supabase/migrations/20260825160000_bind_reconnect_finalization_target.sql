-- Finalize reconnects only against the server-bound target Connection.
...
declare
  ...
  pending_target_connection_id uuid;
  ...
  existing_connection_provider text;
  existing_connection_external_account_id text;
...
begin
  ...
  select provider, purpose, target_connection_id, external_account_id,
    encrypted_access_token, encrypted_refresh_token,
    access_token_expires_at, key_version
  into pending_provider, pending_purpose, pending_target_connection_id,
    pending_external_account_id, pending_encrypted_access_token,
  ...
  if pending_purpose = 'reconnect' then
    if pending_target_connection_id is null then
      raise exception 'reconnect target not found';
    end if;

    perform pg_advisory_xact_lock(
      hashtextextended('connection:' || pending_target_connection_id::text, 0)
    );

    select c.id, c.organization_id, c.store_id, c.provider,
      c.external_account_id, c.status
    into existing_connection_id, existing_connection_organization_id,
      existing_connection_store_id, existing_connection_provider,
      existing_connection_external_account_id, existing_connection_status
    from public.connections as c
    join public.stores as s
      on s.id = c.store_id and s.organization_id = c.organization_id
    where c.id = pending_target_connection_id
    for update of c;

    if existing_connection_id is null
      or existing_connection_organization_id is distinct from p_organization_id
      or existing_connection_provider is distinct from pending_provider
      or existing_connection_external_account_id is distinct from pending_external_account_id
      or existing_connection_status not in ('active', 'disabled') then
      raise exception 'reconnect target is not valid for pending authorization';
    end if;
    ...
    update public.integration_secrets as secrets
    set encrypted_access_token = pending_encrypted_access_token,
      encrypted_refresh_token = pending_encrypted_refresh_token,
      access_token_expires_at = pending_access_token_expires_at,
      token_metadata = '{}'::jsonb,
      key_version = pending_key_version,
      updated_at = now()
    where secrets.connection_id = existing_connection_id
      and secrets.organization_id = p_organization_id;

    if not found then
      raise exception 'reconnect target has no credential material';
    end if;

    insert into public.audit_events (
      organization_id, store_id, actor_membership_id, action,
      ...
      jsonb_build_object(
        ...
        'connectionId', existing_connection_id::text,
        'storeId', existing_connection_store_id::text
      )
    );
    ...
    return query
      select 'reconnected'::text, existing_connection_store_id, existing_connection_id;
    return;
  end if;

  perform pg_advisory_xact_lock(
    hashtextextended(pending_provider || ':' || pending_external_account_id, 0)
  );
  ...
  from public.connections as c
  where c.provider = pending_provider
    and c.external_account_id = pending_external_account_id;
  ...
  if existing_connection_id is not null then
    ...
    if existing_connection_status = 'active' then
      return query
        select 'already_connected'::text, existing_connection_store_id, existing_connection_id;
      return;
    end if;
    ...
    return query
      select 'reactivated'::text, existing_connection_store_id, existing_connection_id;
    return;
  end if;
  ...
  insert into public.connections (
    organization_id, store_id, provider, external_account_id, status
  ) values (
    p_organization_id, new_store_id, pending_provider,
    pending_external_account_id, 'active'
  ...
  insert into public.integration_secrets (
    ...
    new_connection_id, p_organization_id, pending_encrypted_access_token,
    pending_encrypted_refresh_token, pending_access_token_expires_at,
    '{}'::jsonb, pending_key_version
  );
  ...
  insert into public.audit_events (
    ...
    (
      p_organization_id, new_store_id, p_actor_membership_id,
      'store.created', 'store', new_store_id::text, '{}'::jsonb
    ),
    (
      p_organization_id, new_store_id, p_actor_membership_id,
      'integration.connected', 'connection', new_connection_id::text,
      jsonb_build_object(
        'provider', pending_provider,
        'connectionId', new_connection_id::text,
        'storeId', new_store_id::text
      )
    );
...
revoke all on function public.finalize_admin_pending_integration_onboarding(text, uuid, uuid, text)
  from public, anon, authenticated;
grant execute on function public.finalize_admin_pending_integration_onboarding(text, uuid, uuid, text)
  to service_role;
