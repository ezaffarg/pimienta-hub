-- Fix the output-column ambiguity in the reconnect secret upsert.
-- This is forward-only: 20260824210000 is already applied remotely.
...
begin
  ...
  if existing_connection_id is not null then
    ...
    on conflict on constraint integration_secrets_pkey do update
    set
  ...
  values (p_organization_id, new_store_id, pending_provider, pending_external_account_id, 'active')
  ...
  values
    ...
    (
      ...
      jsonb_build_object('provider', pending_provider, 'connectionId', new_connection_id::text, 'storeId', new_store_id::text)
    );
