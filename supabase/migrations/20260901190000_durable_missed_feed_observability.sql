alter table public.integration_event_maintenance_runs
  add column missed_feed_failure_stage text,
  add column provider_calls_attempted_count integer,
  add column provider_calls_succeeded_count integer;

alter table public.integration_event_maintenance_runs
  alter column provider_calls_attempted_count set default 0,
  alter column provider_calls_succeeded_count set default 0,
  add constraint integration_event_maintenance_missed_feed_stage_check check (
    missed_feed_failure_stage is null or missed_feed_failure_stage in (
  ...
  add constraint integration_event_maintenance_provider_calls_check check (
    (provider_calls_attempted_count is null and provider_calls_succeeded_count is null)
    or (
      provider_calls_attempted_count >= 0
      and provider_calls_succeeded_count >= 0
      and provider_calls_succeeded_count <= provider_calls_attempted_count
    )
  );
...
begin
  ...
  if p_received_selected < current_run.received_selected_count
    ...
    or p_provider_calls_succeeded < current_run.provider_calls_succeeded_count then
...
begin
  ...
  select counts.received_backlog, counts.retry_due, counts.processing,
    ...
    latest.provider_calls_succeeded_count
...
comment on column public.integration_event_maintenance_runs.missed_feed_failure_stage is
  'Safe missed-feed failure boundary; never contains provider payloads.';
comment on column public.integration_event_maintenance_runs.provider_calls_attempted_count is
  'Provider requests attempted by missed-feed recovery; null means historical unknown.';
comment on column public.integration_event_maintenance_runs.provider_calls_succeeded_count is
  'Provider responses accepted at the client boundary; null means historical unknown.';
